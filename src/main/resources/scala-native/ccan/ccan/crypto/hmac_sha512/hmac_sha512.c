...
#include "../hmac_sha512/hmac_sha512.h"
#include <string.h>

#define IPAD 0x3636363636363636ULL
#define OPAD 0x5C5C5C5C5C5C5C5CULL

#define BLOCK_U64S (HMAC_SHA512_BLOCKSIZE / sizeof(uint64_t))
		 ...
		 const void *text, size_t text_len)
{
	...
	unsigned char tk[32];
         ...
         * perform inner MD5
         */
        sha512_init(&context);                   /* init context for 1st
                                              * pass */
        sha512_update(&context, k_ipad, 64);      /* start with inner pad */
        sha512_update(&context, text, text_len); /* then text of datagram */
        sha512_done(&context, &hmac->sha);          /* finish up 1st pass */
        /*
         * perform outer MD5
         */
        sha512_init(&context);                   /* init context for 2nd
                                              * pass */
        sha512_update(&context, k_opad, 64);     /* start with outer pad */
        sha512_update(&context, &hmac->sha, 32);     /* then results of 1st
                                              * hash */
        sha512_done(&context, &hmac->sha);          /* finish up 2nd pass */
}
#endif
